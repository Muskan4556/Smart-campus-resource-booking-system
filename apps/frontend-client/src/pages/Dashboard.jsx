...
export default function Dashboard() {
  ...
  const handleBooking = async (resourceId) => {
    ...
    try {
      ...
      await bookResource(
        { userId, resourceId, date: new Date().toISOString().split("T")[0], startTime, endTime },
        token
      );
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .db-nav-btn.admin {
          background: rgba(165,180,252,0.15); border-color: rgba(165,180,252,0.4);
          color: #a5b4fc;
        }
        ...
        .db-card-icon {
          font-size: 28px; line-height: 1; background: var(--db-navy-lt);
        ...
        .db-card-meta-val { font-size: 14px; font-weight: 600; color: var(--db-ink2); }
      ...
      <nav className="db-nav">
        ...
        <div className="db-nav-right">
          ...
          <button className="db-nav-btn" onClick={() => navigate("/booking")}>
            My Bookings
          </button>
            ...
            {resources.map((r) => (
              <div className="db-card" key={r._id}>
                ...
                <div className="db-card-top">
                  {/* FIX: use r.resourceName (matches schema) */}
                  <div className="db-card-icon">{getIcon(r.resourceName)}</div>
                  <span className="db-card-type">{r.resourceId || "Resource"}</span>
                </div>
                {/* FIX: use r.resourceName */}
                <div className="db-card-name">{r.resourceName}</div>
